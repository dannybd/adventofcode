(input =>
  ((draws, initialBoards, drawnPositions, bingo, finalResult) =>
    finalResult(
      // Step through the draws one at a time
      draws.reduce(
        ({boards, winner, winningDraw}, draw, i) =>
          winner
            // The existence of a winner means we hold our value and zip
            // through the remaining reduce steps as a no-op. In normal JS
            // we'd use a `break` or `return` here to jump out of the loop,
            // but we can't do that.
            ? {winner, winningDraw}
            : (updatedBoards =>
                ({
                  winner: updatedBoards
                    .find(board => bingo(drawnPositions(board))),
                  winningDraw: draw,
                  boards: updatedBoards,
                })
              )(
                // In each step, replace the values which are drawn with null
                boards.map(board => board.map(n => n !== draw ? n : null)),
              ),
